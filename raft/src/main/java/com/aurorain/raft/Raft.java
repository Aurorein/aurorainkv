...
import com.aurorain.raft.model.RaftState;
import com.aurorain.raft.service.RaftService;
import com.aurorain.commonmodule.utils.RpcUtils;
import com.aurorain.raft.web.util.SpringContextUtil;
...
public class Raft implements RaftService {
    ...
    @JsonIgnore
    private transient RaftState state;

    private SimpMessagingTemplate messagingTemplate;
    private ScheduledExecutorService logSenderExecutor;

    private Queue<String> logCache = new ConcurrentLinkedQueue<>();
     ...
     * @param channel
     */
    public void init(Map<Integer, RaftService> peers, int me, Persister persister, Channel<ApplyMsg> channel, boolean isShardMaster) {
        int n = peers.size();
        state = new RaftState();
        // 初始化状态
...
//        state.setReplicatorCond(new Condition[n]);
        ...
        state.setShardMaster(isShardMaster);
        resetElectionTimer();
        resetHeartbeatTimer();
...
//        }
        // 初始化定时任务线程池
        this.logSenderExecutor = Executors.newSingleThreadScheduledExecutor();

        // 启动定时任务，每5秒尝试发送一次缓存日志
        this.logSenderExecutor.scheduleAtFixedRate(this::trySendCachedLogs,
    ...
    public RequestVoteResponse requestVote(RequestVoteRequest request) {
        ...
        if (state.getVotedFor() == -1 || state.getVotedFor() == request.getCandidateId()) {
            ...
            log("raft:{} 同意投给raft:{}一票在任期:{}", state.getMe(), request.getCandidateId(), getTerm());
            ...
            switch (state.getState()) {
                case RaftConstant.FOLLOWER:
                    if (electionTimeout()) {
                        ...
                        log("raft:{} 心跳超时，由Follower转变为Candidate，开始进行选举", state.getMe());
                        doElection();
                        resetElectionTimer();
                    }
                    break;
...
//                        log.info("raft:{} starts a re-election", state.getMe());
                        log("raft:{} 心跳超时，由Candidate转变为Candidate，开始进行选举", state.getMe());
                        doElection();
                        resetElectionTimer();
                    }
                    break;
    ...
    public void doElection() {
        log("raft:{} 变成候选者开始发起选举，现在的任期是{}", state.getMe(), state.getCurrentTerm());
        ...
        for (int i : state.getPeers().keySet()) {
            ...
            tasks.put(i, CompletableFuture.supplyAsync(() -> {
                log("raft:{} 向 raft:{} 发起投票请求，现在的任期是{}", state.getMe(), peer, state.getCurrentTerm());
...
//                log.info("raft{} {}", peer, response);
                ...
                if (response.getTerm() == state.getCurrentTerm() && state.getState() == RaftConstant.CANDIDATE) {
                    if (response.isVoteGranted()) {
                        log("raft:{} 收到 raft:{} 的投票", state.getMe(), peer);
                        lock.unlock();
                        return 1;
...
//        log.info("id {}统计票数", state.getMe());
        ...
        for (int i : state.getPeers().keySet()) {
            ...
            if (votes > n / 2) {
                log("raft:{} 收到大多数投票:{} > {} / 2，成功当选 Leader，任期是{}", state.getMe(), votes, n, state.getCurrentTerm());
                leaderInit();
                return;
            }
        }
//        log.info("id {} 的票数统计: {}",state.getMe(), votes);
        log("raft:{} 未能当选 Leader，当前票数是{}", state.getMe(), votes);
...
//        log.info("raft:{} 作为Leader开始发起AppendEntries，现在的任期是{}", state.getMe(), state.getCurrentTerm());
        for (int i : state.getPeers().keySet()) {
            if (i == state.getMe()) {
                continue;
            }
//            log.info("appendEntries to server {}", i);
            int server = i;
//            log.info("raft: {} append entries to {}", state.getMe(), server);
            ...
            if (getFrontLog().getIndex() > state.getNextIndex().get(server) - 1) {
                ...
                // 日志复制线程
//                log.info("raft:{} 作为Leader向raft:{}AppendEntries，现在的任期是{}", state.getMe(), i, state.getCurrentTerm());
...
//        log("raft:{} 向 raft:{} 发送日志复制请求，前一条日志索引是{}，任期是{}", state.getMe(), server, request.getPreLogIndex(), request.getTerm());
...
//            log.info("raft:{}'s term is larger than raft:{}'s", server, state.getMe());
            log("raft:{} 发现 raft:{} 的任期更大，转变为 Follower", state.getMe(), server);
            state.setCurrentTerm(response.getTerm());
            state.setVotedFor(-1);
            persist();
            turnTo(RaftConstant.FOLLOWER);
...
//            log("raft:{} 成功复制日志到 raft:{}，更新 nextIndex 为{}", state.getMe(), server, request.getPreLogIndex() + entries.length + 1);
...
//                    log.info("commit check; {} rafts commit to index {}", cnt, i);
                ...
                if (cnt > n / 2) {
                    log("raft:{} 提交日志到索引{}，任期是{}", state.getMe(), i, state.getCurrentTerm());
...
//        log.info("raft:{} doesn't have half replicated from commitIndex {} to lastLogIndex {} now", state.getMe(), state.getCommitIndex(), getLastLog().getIndex());
    ...
    public void turnTo(int turn) {
        switch (turn) {
            case RaftConstant.LEADER:
                log("raft:{} 转变为 Leader，任期是{}", state.getMe(), state.getCurrentTerm());
                leaderInit();
                break;
            case RaftConstant.CANDIDATE:
                log("raft:{} 转变为 Candidate，任期是{}", state.getMe(), state.getCurrentTerm() + 1);
                ...
                break;
            case RaftConstant.FOLLOWER:
                log("raft:{} 转变为 Follower，任期是{}", state.getMe(), state.getCurrentTerm());
...
//        log.info("raft:{}: appends a cmd:{}, lastLogIndex:{}", state.getMe(), cmd, getLastLog().getIndex());
    ...
    public void kill() {
        state.getDead().set(1);
        if (logSenderExecutor != null) {
            logSenderExecutor.shutdownNow();
        }
    }
    ...
    private void log(String format, Object... args) {
        ...
        try {
            ...
            System.err.println(timestamp + " 日志格式化失败: " + e.getMessage());
            return;
        }
...
//        System.out.println("[" + timestamp + "] [RAFT日志] " + message);
        ...
        if (message == null || message.trim().isEmpty()) {
            System.err.println(timestamp + " 试图发送空消息到WebSocket");
            return;
        }

        // 4. 将带时间戳的消息添加到缓存
        logCache.offer("[" + timestamp + "] " + "[term:" +state.getCurrentTerm() + "] " +message);

        // 5. 如果WebSocket可用，立即尝试发送
        // trySendCachedLogs();
    }
     ...
     * 尝试发送缓存中的日志
     */
    private synchronized void trySendCachedLogs() {
        if (messagingTemplate == null || logCache.isEmpty()) {
            return;
        }

        try {
            Iterator<String> iterator = logCache.iterator();
