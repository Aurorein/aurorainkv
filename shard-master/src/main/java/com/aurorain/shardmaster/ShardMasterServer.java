...
import com.aurorain.shardmaster.constant.Message;
...
//        groups.put(1, list);
        ...
        Raft raft = new Raft();
        state.setRaft(raft);
        raft.init(peers, me, persister, state.getChannel(), true);
    ...
    public ShardCommandResponse requestCommand(ShardCommandRequest request) {
        ...
        // lock
//        log.info(Thread.currentThread().getId() + ":" + "requestCommand lock");
        state.getLock().lock();
...
//            log.info("query execute...");
        ...
        log.info("shard master: correct leader, leader id :{}!", state.getMe());
        state.getLock().lock();
        ...
        while (LocalDateTime.now().isBefore(time)) {
            ...
            log.info(Thread.currentThread().getId() + ":" + "wait for channel");
            if ((res = channel.read()) != null) {
                log.info(Thread.currentThread().getId() + ":" + "channel get read");
                // 销毁通道
    ...
    public void applier() {
        ...
        while (!killed()) {
            ...
            if ((applyMsg = channel.read()) != null) {
                log.info(Thread.currentThread().getId() + ":" + "{}",applyMsg);
                if (applyMsg.isCommandValid()) {
                    log.info(Thread.currentThread().getId() + ":" + "isLeader:" + state.getRaft().isLeader());
                    state.getLock().lock();
                    // 检查消息是否过时
                    ...
                    log.info(Thread.currentThread().getId() + ":" + "shard master: received applyMsg: type {}, seqId {}, clientId {}", args.getType(), args.getSeqId(), args.getClientId());
                    ...
                    switch(args.getType()) {
                        case JOIN: {
                            log.info(Thread.currentThread().getId() + ":" + "进入join逻辑~");
                            join((JoinArgs) args);
                        ...
                        case MOVE: {
                            log.info(Thread.currentThread().getId() + ":" + "进入move逻辑~");
                            move((MoveArgs) args);
...
//                        }
                    ...
                    log.info(Thread.currentThread().getId() + ":" + "return response: {}", response.toString());
                    // 检查状态
...
//                    log.info("indexAndTerm :{} and {}", applyMsg.getCommandIndex(), applyMsg.getCommandTerm());
    ...
    private void join(JoinArgs joinArgs) {
        ...
        if(joinArgs.getServers().isEmpty()) return;
...
//        adjustConfig(config);
    ...
    private boolean killed() {
        return state.getDeadValue().get() == 1;
    }
    ...
    public ShardServerState getState() {
        return state;
    }
}
